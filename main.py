...
# ========= CONFIG =========
BOT_TOKEN = os.getenv("8371104768:AAHi2lv7CFNFAWycjWeUSJiOn9YR0Qvep_4")  # טוקן מהסביבה (לא לשמור בקוד)
...
SESSION.headers.update({"User-Agent": "TelegramPostBot/1.2"})
...
def post_to_channel(product):
    try:
        ...
        if video_url.endswith('.mp4'):
            bot.send_video(CHANNEL_ID, video_url, caption=post_text)
        else:
            bot.send_photo(CHANNEL_ID, image_url, caption=post_text)

    except Exception as e:
        ts = datetime.now(tz=IL_TZ).strftime('%Y-%m-%d %H:%M:%S %Z')
        print(f"[{ts}] Failed to post item {product.get('ItemId', 'ללא מספר')}: {e}")
        raise
...
def run_sender_loop():
    ...
    while True:
        ...
        try:
            ...
            # הסרה מהתור רק אם הצליח
            write_products(PENDING_CSV, pending[1:])
        except Exception as e:
            # דילוג על פריט בעייתי (לוג) והמשך לתור הבא
            ts = datetime.now(tz=IL_TZ).strftime('%Y-%m-%d %H:%M:%S %Z')
...
CHANNEL_ID = "@nisayon121"       # ← עדכן כאן (למשל: "@my_channel")
ADMIN_USER_IDS = set(535299257)  # ← מומלץ להגדיר user id שלך: {123456789}
